...
export default function HomePage() {
  ...
  return (
    <>
      {/* ── Hero ── */}
      <Section className="relative flex items-center min-h-[70vh] pt-20" style={{ background: "var(--hero-bg)", color: "var(--hero-text)" }}>
        ...
        <Container className="relative z-20 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center py-12 md:py-24">
          <div className="flex flex-col items-start text-left gap-6">
            <FadeIn>
              <span
                className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold tracking-widest uppercase mb-2"
                style={{ background: "var(--hero-badge-bg)", border: "1px solid var(--hero-badge-border)", color: "var(--accent)" }}
              >
                <ShieldCheck size={14} /> Finance · Compliance · Leadership
              </span>
            </FadeIn>

            <FadeIn delay={0.1}>
              <h1 className="font-serif text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight tracking-tight">
                INSEAD Master in <span className="italic">Finance</span> | Educator | Compliance Leader
              </h1>
            </FadeIn>

            <FadeIn delay={0.2}>
              <p className="text-md md:text-lg max-w-xl leading-relaxed" style={{ color: "var(--hero-muted)" }}>
                Savita Dubey is a Chartered Accountant and Master in Finance graduate from INSEAD with over 15 years of experience across audit, banking, and regulatory compliance.
              </p>
            </FadeIn>
